...
| 原始需求 | 实现与证据 | 结论 |
| --- | --- | --- |
| 触摸屏 | Tab5 诊断固件已在 1280×720 全屏读取多点触摸并显示正确触点；配对键盘使用真实按下边沿、40 ms 释放去抖和局部数字刷新；列表除 8 像素圆形容差外还锁定 M5 驱动的 flick/drag，只有明确的 `wasClicked()` 才进入会话，丢失释放帧直接取消；v0.4.2 将滚动从整块重传改为屏内搬移与暴露条带补绘，刷新目标为 16 ms | 代码、自动化与 v0.4.2 部署已完成；最终滑动手感待用户复验 |
...
| Chibi Skadi、菲比 Q 版主题与会话详情 | `chibi-skadi` 系列启用深海蓝、冰晶青、珊瑚粉主题；`feibi` 启用深莓紫、奶油白、香槟金、紫罗兰与玫瑰粉主题。状态栏、电量、链路、额度、Token、卡片、按钮和滚动条均随 Pet 切换；含 Git 信息的线程显示为项目文件夹卡，无 Git 信息的线程显示为对话气泡卡；轻点卡片后通过只读 `thread/read` 展开最近 12 条用户/助手消息，过滤 reasoning、工具参数与内部事件；v0.4.2 进一步放慢全部动画，Skadi 的界面与 LLM 对话统一称用户为“博士” | 两套主题的电脑端视觉验收、协议测试、真实固件构建和 v0.4.2 部署已完成；真机主题观感仍需用户目视确认 |
...
| 蓝牙或其他无线方式 | ESP32-C6 BLE 已在 2026-07-27 完成拔线状态同步；2026-07-29 v0.4.1 升级后无需重新配对，重载 Bridge 后真机再次回报 USB+BLE 双链路。BLE 可承载任务列表、只读会话详情、Pet 选择、审批和遥测；Pet、语音和图片走 USB/Wi‑Fi | BLE 轻量无线链路已完成；自动恢复长稳与 Wi‑Fi 弱信号恢复待真机 |
...
- v0.4.1 将会话进入收紧为 M5 驱动确认的点击，移除无释放帧时的补开逻辑；原生测试要求所有动画循环不少于 1.5 秒，Idle 动作前后各停留 1.8 秒，Pet 帧区域从 y=108 下移到 y=124。
- 真实 ESP32-P4 v0.4.1 发布构建通过：应用占用 6,371,676 / 6,553,600 Bytes（97.2%）；整机镜像 16,711,680 Bytes，SHA-256 `2cf434b5115db960255de65b7fcb7776cd4a5c2522489af64be3341be84f0b9a`。
- v0.4.1 已用非擦除升级写入真机，所有写入分区通过哈希校验；设备无需重新配对并回报固件 0.4.1、协议 v4、USB+BLE、microSD 与离线中文语音健康。
- v0.4.2 将任务列表和会话详情的滚动刷新从每步重传 780×390/780×500 视口，改为显示缓冲区原地搬移并只上传新暴露条带与滚动条；目标刷新间隔从 33 ms 降至 16 ms。全部动画循环不少于 2.2 秒，Idle 动作前后各停留 2.5 秒；Skadi 对话使用独立“博士”角色线程。
- 真实 ESP32-P4 v0.4.2 发布构建通过：应用占用 6,372,108 / 6,553,600 Bytes（97.2%）；整机镜像 16,711,680 Bytes，SHA-256 `502e2e6ca220baf9a1a4a398eb4efa4edd14e86741c5423af724f566354a8399`。
- v0.4.2 已用非擦除升级写入真机，所有写入分区通过哈希校验；设备无需重新配对并回报固件 0.4.2、协议 v4、USB+BLE、microSD 与离线中文语音健康，4 个 Pet 与 12 条最新任务均正常。

## 剩余真机与产品化清单

只有以下项目没有办法在无实物状态下诚实关闭：

1. v0.4.2 已写入；目视确认任务列表和会话详情的滑动手感、宠物循环节奏及 Skadi“博士”称呼后，继续实测语音转写/回复/命令确认和摄像头画面/色彩/观察回复。
