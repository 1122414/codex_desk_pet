...
| Chibi Skadi Q 版主题与会话详情 | `chibi-skadi` 系列 Pet 启用独立深海蓝、冰晶青、珊瑚粉主题；状态栏、电量、链路、额度、Token、卡片、按钮和滚动条使用专属图形语言。含 Git 信息的线程显示为切角项目卡，无 Git 信息的线程显示为圆角对话卡；轻点卡片后通过只读 `thread/read` 展开最近 12 条用户/助手消息，过滤 reasoning、工具参数与内部事件 | 电脑端浏览器视觉验收、协议测试、固件构建和 v0.3.0 真机部署已完成；真机主题触摸与屏幕观感仍需用户目视确认 |
...
1. v0.3.0 已写入；目视确认 Skadi 主题、项目/对话卡和会话详情触摸后，继续实测语音转写/回复/命令确认和摄像头画面/色彩/观察回复。
