...
| 10 | 可发布的 Bridge、协议与固件 | 项目/Bridge/App Server 客户端/固件均为 0.3.0，设备协议为 v5；发布清单固定 ESP32-P4 偏移、大小和逐文件 SHA-256 | `npm run check`、虚拟设备、稳定性、doctor、Codex 冒烟、真实 PlatformIO 发布构建及 Tab5 P4 rev1.3 整机刷写/哈希校验 | 真机已烧录，待 24 小时验收 |
...
- `npm run check`：174/174 测试和固件核心检查通过。
...
- v0.3.0 完整工厂镜像由真实 ESP32-P4 工具链构建并逐组件复验：16,711,680 Bytes，SHA-256 `2b278bd62f38543dd7b10ad4dd82e84fea27fd68c944571e41221c32a64eae57`。
- `npm run acceptance:hardware` 的受控 Bridge 验收通过：记录器可以原子保存、接收停止信号、从检查点恢复，并对五轮会话、六组动作、重复、失败、Wi‑Fi 重连、RSS、电量和温度作出机器可复验判定。
- Tab5 P4 rev1.3 已写入 v0.3.0，整片写入哈希校验通过；协议 v5 的 HMAC、会话加密和 USB 可靠消息已在真机闭环。设备上报固件 0.3.0、全部能力、语音/存储健康以及电量和温度遥测。
- `chibi-skadi-v2` 仍为当前 Pet；Bridge 在真机重新认证后恢复该选择。大规格 Pet 改为 ACK 驱动的流式传输，不再一次性缓存全部 28 MB 设备资源。

## 已连接开发板的剩余最终清单

固件烧录、配对、USB 加密会话、版本/能力和基础遥测已经完成。以下结论仍依赖当前连接的 Tab5，不能用虚拟设备代替：

1. 配置并实测 Wi-Fi 链路、断开和恢复。
