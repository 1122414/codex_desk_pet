...
- `version`：当前固定为 `2`；未知版本必须拒绝，不能按旧版本猜测。
...
配对密钥只允许在 USB 或启用 Secure Connections + MITM 的 BLE 本地配对链路下发，不会进入浏览器 API、设备列表或日志。BLE 配网特征也要求加密和 MITM；设备屏幕上的 6 位配网码同时是蓝牙 passkey，成功写入 Wi‑Fi 后立即轮换并重启生效。
...
- GCM nonce 是方向密钥派生的 4 字节前缀加 8 字节大端序消息序号；同一方向的序号不得复用。
...
- 凭据不得写入日志或前端静态文件。
- 协议 v2 已提供等价的应用层 AES‑256‑GCM 加密，并已通过篡改、方向隔离和明文降级测试；任务与审批 payload 不再以明文出现。
