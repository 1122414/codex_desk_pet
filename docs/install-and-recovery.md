...
Tab5 已完成整机烧录、USB 枚举、应用升级、屏幕显示、全屏多点触摸诊断、BLE 拔线状态同步和中文语音分区完整性校验。v0.2.0 已离线构建但尚未写入设备；用户回来后继续完成：

- 写入 v0.2.0 工厂镜像，确认原配对凭据是否保留；如因分区布局升级需完整恢复，再重新配对一次。
- 按住“对话”和“命令”录音，验证转写、回复、命令二次确认、扬声器音量和中断恢复。
