...
- 点一下“对话”或“命令”后，设备暂停扬声器并以 16 kHz 单声道录音，经已认证的 USB/Wi‑Fi 加密链路送入 Bridge；检测到说话结束后自动提交，再点一下可提前结束。Bridge 在电脑本地用 `whisper-cli` 转写后恢复扬声器。
...
真机前已验证：官方文件哈希、C++ ABI 兼容层、静态库链接、Flash 分区尺寸、本地 Whisper 转写适配、三轮自动关怀对话、停止/掉线恢复、命令确认边界和完整 ESP32-P4 固件构建。仍需在 v0.3.0 真机验证：麦克风增益、静音检测、转写延迟、扬声器音量、破音、TTS 后自动录音的切换时序和实际中文听感。
