...
- `voice_data` 在首次初始化时以内部 RAM 小块从 Flash 读取、流式校验完整 SHA‑256，再存入 Tab5 的 PSRAM，并与同一套 `xiaoxin` 发音表绑定。这样避免大数据分区映射和 PSRAM 大块校验兼容性问题；分区缺失、数据损坏、TTS 初始化失败或播放失败时自动使用有区分度的非阻塞音型。
