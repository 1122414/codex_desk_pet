...
设备连接 `ws://<电脑局域网地址>:4318/device/ws`。控制面板仍只监听 `127.0.0.1:4317`。设备协议 v2 在认证后会加密包括任务、审批、ACK、心跳和 Pet 资源在内的全部 payload；明文降级、方向错误、元数据或密文篡改都会被拒绝。外层消息类型和时序元数据不加密，因此设备端口仍不应暴露到公网。
...
- “设备配对”生成的 6 位码只能使用一次，并在 5 分钟后过期；首次协议配对只接受 USB 或已加密的 BLE。Web Bluetooth 配网还会触发系统蓝牙配对，并要求输入设备屏幕上的独立配网码。
