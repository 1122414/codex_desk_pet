...
- 最近记录会按 Git 工作区区分“项目”和“纯对话”；两类使用不同卡片语言，点击后可只读查看最近 12 条用户/助手消息，推理、工具调用和图片不会发送到设备。选择 `chibi-skadi` 或其变体时，Tab5 与电脑控制页会启用配套的冰海 Q 版主题、切角项目卡、圆角对话卡和分段电量图标。
...
- 在“项目与对话”列表点任一记录，可展开最近对话；项目显示工作区与切角文件夹样式，纯对话显示独立气泡样式。
