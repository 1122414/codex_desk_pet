...
- 最近记录会按 Git 工作区区分“项目”和“纯对话”；两类使用不同卡片语言，点击后可只读查看最近 12 条用户/助手消息，推理、工具调用和图片不会发送到设备。选择 `chibi-skadi` 或其变体时启用冰海 Q 版主题；选择 `feibi` 时启用深莓紫、奶油白、香槟金和紫罗兰主题。两套主题都覆盖 Tab5 与电脑控制页的背景、项目/对话卡、电量和状态图标。
...
- 在“项目与对话”列表轻点任一记录，可展开最近对话；项目显示工作区与切角文件夹样式，纯对话显示独立气泡样式。列表滑动使用独立手势锁，任意方向移动超过 8 像素后，松手不会打开项目或对话。
...
- 完整 Tab5 v0.4.0 固件已经通过真实 ESP32-P4 工具链编译并写入真机；普通升级保留 NVS 中的配对与网络配置。本轮重载 Bridge 后，设备回报协议 v4、USB+BLE 双链路、microSD 与离线中文语音数据健康；菲比已通过 USB 触发设备资源安装后恢复 `chibi-skadi-v2`。摄像头画面、Wi‑Fi 射频、扬声器听感、电量曲线和 72 小时长稳仍需物理验收。
